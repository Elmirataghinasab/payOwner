...
async function getNativeBalanceInUSD(chainId,walletAddress) {
  ...
  try {
    ...
    const nativeToken = balances.find(token => token.contract_address === '0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee');
...
async function getNativeBalance(chainId,walletAddress) {
  ...
  try {
    ...
    if (nativeToken) {       
      console.log(`Native Balance: ${nativeToken.balance / 10 ** nativeToken.contract_decimals} ${nativeToken.contract_ticker_symbol}`);
...
async function getBalance(chainId,walletAddress,symbol) {
    ...
    console.log(`Nativebalance in ${chainId} network for account: ${walletAddress} is ${nativeBalance} `)
    console.log(`USD balance of ${walletAddress} in ${chainId} network is ${balanceInUSD}`)
...
async function getPOLNativeBalanceInUSD(chainId,walletAddress) {
  ...
  try {
    const response = await axios.get(url);
    const balances = response.data.data.items;
    const nativeToken = balances.find(token => token.contract_address === '0x0000000000000000000000000000000000001010');
...
main("0x81878429C68350DdB41Aaaf05cF2f03bf37e72D5")
